...
class VisionTransformer(nn.Module):
    ...
    def mask_tokens(self, orig_image, feats):
        ...
        """
        img_unnorm = orig_image * 0.5 + 0.5  # convert to [0, 1]
        _, _, ph, pw = self.patch_embed.proj.weight.shape
        with torch.no_grad():
            # (batch, 3, h, w) -> (batch, 1, num_patch, num_patch)
            # It seems to be an average pooling, so label is the average value of an image patch
    ...
    def visual_embed(self, _x, max_image_len=200, mask_it=False):
        ...
        x = self.patch_embed(_x)  # (batch, emb_dim, num_patch, num_patch)
        ...
        x = x.flatten(2).transpose(1, 2)  # (batch, num_patch * num_patch, emb_dim)
        patch_index = (
            torch.stack(
                torch.meshgrid(
